...
class TratamentoColunaAnoSimples():
    def tratar(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.melt(id_vars=id_vars, value_vars=value_vars,
                     var_name='ANO', value_name=value_name)

        # Renomear as colunas 'control' para 'Tipo' e 'produto' para 'PRODUTO'
        df = df.rename(columns={'control': 'CONTROL', 'produto': 'PRODUTO',
                                'Produto': 'PRODUTO', 'cultivar': 'CULTIVAR'})
...
class TratamentoColunaAnoDuplo():
    def tratar(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df2.columns = [col.replace('.1', '') if col not in [
            'Id', 'País'] else col for col in df2.columns]
        value_vars = df1.columns[2:]
        # Realizar o melt em df1
        df1_melted = df1.melt(
            id_vars=['Id', 'País'],  value_vars=value_vars, var_name='Ano', value_name='KG')

        value_vars = df2.columns[2:]
        # Realizar o melt em df2
        df2_melted = df2.melt(
            id_vars=['Id', 'País'], value_vars=value_vars, var_name='Ano', value_name='US$')
